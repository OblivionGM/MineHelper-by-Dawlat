...
- Подсчёт общей стоимости добытого по кастомизируемым ценам
...
- Настраиваемые бинды на рутинные задачи
...
3. Для обновления скрипта достаточно запустить SA:MP с установленным MineHelper — он сам проверит наличие новых версий.

Примечание: Если у вас отсутствует библиотека `json.lua` в пути `moonloader/lib` скрипт работать не будет.
Установите её, скачав и переместив файл `lib/json.lua` из текущего репозитория по указанному пути.
...
### Версия 1.5.10 (текущая)

- Исправлен учёт ресурса "Уголь"
- Исправлен метод использования еды по нажатию кнопки
- Исправлена система взаимодействия с новым инвентарем
- Переработан сканер инвентаря
- Доработаны технические записи в консоли для более точного определения проблем у конечного пользователя

### Версия 1.5.9

- Добавлена поддержка использования еды с новым инвентарем
- Добавлен сканер инвентаря
- Обновлен раздел "Информация"

### Версия 1.5.8

- Переработана система меток

### Версия 1.5.7

- Исправлены мелкие баги с учётом ресурсов
